/*
  1. Зачем DomContentLoaded? Логичнее просто перенести подключение скриптов вниз в .html файле.
     А еще лучше всё же подключать через vue-cli (или сборщик), а не через скриптом.

  2. Всё очень сложно выглядит в одном компоненте. Стоило бы разбить на компоненты попроще
  3. Часть переменных из data  можно вынести в computed, например textGameVictory (остальные не проверял).
  4. Неправильно обращаться к dom-елементам напрямую (document.getElementById(), classList итд).
     В идеале здесь нужно просто разбить всё на компоненты, тогда проблема уйдёт сама собой. Ну и у вью есть директива
     ref для доступа к элементам (но в данной задаче при разбиении на компоненте это тдаже не понадобится)
  5. Везде нестрогое сравнение :(
 */

const gameBS = new Vue({  //зачем var?
  el: "#gameBS",
  data: {
    ...
    /**
     * зачем timer лежит в data? Он же не меняется. Можно его вынести в $options (пример ниже)
     */
    ...
    /**
     * textGameVictory можно в computed
     */
  ...
  methods: {
    ...
    drawShips: function (cells, valueShips, valueAdds, player, ships) {
      ...
      function drawShipsOne(lengthShips, s) {
        ...
        /**
         * Я бы для итерирования использовал встроенные функции массива (.forEach(), .map(), .reduce()).
         * for - очень многословно, усложняет чтение
         */
    ...
    fireComputer: function () {
      ...
      if (this.valueShipsUser.indexOf(addressClick) >= 0) {
        ...
        for (let s = 0; s < 10; s++) {
          /**
           * Какая-то сложная и неочевидная структура хранения данных в массивах.
           * Вообще не ясно, что тут за s? почему мы его итерируем до 10? Почему берём первый элемент, а потом нулевой?
           * Стоит подумать, чтобы использовать более явные структуры (обьект, Map)
           */
        ...
        /**
         * Лучше не использовать нестрогое сравнение
         */
    ...
    modalOpenClose: function (textHeader, name, active) {
      ...
      /**
       * Можно же просто   this.isActiveModal = !this.isActiveModal;
       */
  ...
  mounted() {
    ...
    /**
     * Здесь не понял: при инициализации Data-свойств заданы какие-то значение, но при монтировании компонента они
     * всё равно переопределяются (внутри restart()). Зачем так?
     */
    this.restart();
  },
  ...
  computed: {
    ...
    /**
     * Ниже 2 компьютед-свойства делают по сути одно и то же: возвращают одинаковый обьект (только ключи разные)
     * в зависимости от одного значения
     *
     * Ну и вообще не очень понятно, зачем нужен обьект вида:
     * {
          activeModalOverlay: this.isActiveModal === true,
          inActiveModalOverlay: this.isActiveModal === false,
        };
     * У него просто эти 2 свойства будут отличаться друг от друга. Можно же использовать одну булеву переменную
     * и использовать её для активного класса, а второе состояние будет просто дефолтно задано в стилях
     */
